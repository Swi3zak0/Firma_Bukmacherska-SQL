...
BEGIN
    ...
    -- 2. Złagodzona walidacja daty (tylko Warning w logach, żeby nie blokować importu)
    IF :NEW.data_meczu < SYSDATE THEN
        -- Możemy tu nic nie robić, albo logować. Ważne, żeby nie było RAISE_ERROR przy imporcie historycznym.
        NULL;
    END IF;
